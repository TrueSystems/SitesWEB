...
ASPxGridObject.ShowSelection = function(tr){	
	...
	if(this.selectedRowLevel < this.groupColumns.length){				
		...
		while(this.columns[this.columns.length - offset].type == "RowBtnColumn")
			offset ++;		
		this.savedCellsColors[this.savedCellsColors.length] = new CellColor(tr.cells[tr.cells.length - offset].style.color, tr.cells[tr.cells.length - offset].style.backgroundColor);
		...
		for(var i = 0; i < this.columns.length; i ++){
			if(this.columns[i].type == "DataColumn" || this.columns[i].type == "TemplateColumn"){
				this.savedCellsColors[this.savedCellsColors.length] = new CellColor(tr.cells[i].style.color, tr.cells[i].style.backgroundColor);
...
ASPxGridObject.MoveTo = function(tr){			
	if(tr != this.focusedTR){		
		...
		this.focusedTR = tr;	
		this.focusedIndex = ArrayIndexOf(tr.parentElement.children, tr);		
		if(this.focusedTR != null){
				...
				if(this.columns[i].type != ""){
					...
					if(header != null){
						...
						if(x > headerX && x < headerX + header.offsetWidth){
							...
							var scroller = document.all(this.name + "scroller");	
							if((this.columns[i].type == "DataColumn" || this.columns[i].type == "TemplateColumn") && (scroller == null || targetX > scroller.scrollLeft && targetX < scroller.scrollLeft + scroller.offsetWidth)){
	...
	if(grouppanel != null){		
		...
		if(x > grouppanelX && x < grouppanelX + grouppanel.offsetWidth && y > grouppanelY && y < grouppanelY + grouppanel.offsetHeight){
			if(this.groupColumns.length == 0){
				...
				var targetIndex = 0;
				var groupHeaders = grouppanel.cells[0].children[0].rows[0];
				for(var i = 0; i < groupHeaders.children.length - 1; i ++){															
					var header = groupHeaders.children[i];
					if(header != null){
						var headerX = AbsoluteX(header, this.name, false, true);
						if((x > headerX && x < headerX + header.offsetWidth) || (i == groupHeaders.children.length - 2)){
							if(i == 0){
								i = 1;
								header = groupHeaders.children[i];
...
ASPxGridObject.EditPageSize = function(img){	
	var edit;
	edit = document.all(this.name + "PageSizeEdit");				
	edit.disabled = false;
	edit.focus();
	if(img.tagName != "IMG")
		img = img.children[0];
...
ASPxGridObject.CancelPageSize = function(cancel){
	var edit = document.all(this.name + "PageSizeEdit");
	if(cancel)
...
ASPxGridObject.MoveBy = function(direction){	
	var newtr = null;
	if(document.all(this.name + "FocusedRow").value != -1){			
		...
		if(oldFocused != null){
			...
			if(direction && this.focusedIndex < maxIndex){
				newtr = rows[this.focusedIndex + 1];
			}
