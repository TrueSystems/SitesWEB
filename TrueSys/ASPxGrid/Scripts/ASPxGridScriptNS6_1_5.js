...
function showDragHeader(header, drgHdr, isGrouped, borderCollapse){
	...
	drgHdr.style.left = absoluteX(header) + (isMozilla ? (borderCollapse || isGrouped ? 0 : 1) : 0);
	drgHdr.style.top = absoluteY(header) + (isMozilla ? (borderCollapse || isGrouped ? 0 : 1) : 0);
	drgHdr.innerHTML = header.innerHTML;					
	drgHdr.style.width = header.offsetWidth - (isMozilla ? (isGrouped ? 1 : (borderCollapse ? 1 : 2)) : 2); 
	drgHdr.style.height = header.offsetHeight - (isMozilla ? (isGrouped ? 1 : (borderCollapse ? 1 : 2)) : 2); 
}
